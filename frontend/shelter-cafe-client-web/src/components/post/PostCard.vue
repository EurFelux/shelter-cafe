...
<template>
  <CafeLink
    class="card card-side bg-base-200 m-4 h-48 w-full cursor-pointer transition-transform hover:scale-[1.01]"
    :to="to"
    :aria-label="title"
  >
    <div class="card-body">
      <h2 class="card-title line-clamp-1">{{ title }}</h2>
      <p class="text-base-content line-clamp-3 text-sm">{{ description ?? content }}</p>
      <div class="flex">
